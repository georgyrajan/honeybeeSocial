...
public class GlobalExceptionHandler {
	...
	protected ResponseEntity<BaseResponseDTO<Object>> handleGenericException(Exception ex, WebRequest request) {
		...
		return new ResponseEntity<>(
				SurveyCustomerAnswerUtils.wrapErrorResponse(null, ex.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR),
				HttpStatus.INTERNAL_SERVER_ERROR);

	}

	@ExceptionHandler(CustomerException.class)
	protected ResponseEntity<BaseResponseDTO<Object>> handleAdminValidationException(CustomerException ex,
			WebRequest request) {
		LOGGER.error("AdminValidationException error occured with message %s", ex.getMessage());
		return new ResponseEntity<>(
				SurveyCustomerAnswerUtils.wrapErrorResponse(null, ex.getMessage(), HttpStatus.BAD_REQUEST),
	...
	protected ResponseEntity<BaseResponseDTO<Object>> handleConstraintViolation(ConstraintViolationException ex) {
		...
		return new ResponseEntity<>(
				SurveyCustomerAnswerUtils.wrapErrorResponse(null, errors.toString(), HttpStatus.BAD_REQUEST),
				HttpStatus.BAD_REQUEST);
	}
