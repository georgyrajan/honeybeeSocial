...
<body>
<script>
function saveCustomerAnswer(type) {
		...
		if(type=='submit'){
			customerDTOJS.isComplete =true;
		}
		var xhr = new XMLHttpRequest();
		xhr.open("POST", "/saveanswer");
		xhr.setRequestHeader("Accept", "application/json");
		xhr.setRequestHeader("Content-Type", "application/json");
		xhr.setRequestHeader("Auth-JWT", document.getElementById("jwtvalue").value);
...
<form onsubmit="void(0)" style="border:1px solid #ccc">
  <div class="container" id="surveycontainer" style="border:1px solid black;margin-left:auto;margin-right:auto;margin-top: auto;">>
	...
	<c:forEach var="question" items="${questions}">
		...
		<div class="questions">
				...
				<c:if test="${empty answer.answer}">
		         <input type="text" placeholder="${question.text}" id="${answer.id}-${question.code}" name="${question.name}">
		         </c:if>
		         <c:if test="${not empty answer.answer}">
		         <input type="text" placeholder="${answer.answer}" id="${answer.id}-${question.code}" value="${answer.answer}" name="${question.name}">
		         </c:if>
		         </c:forEach>
		  </div>
		 </c:if>
	...
	<input type="hidden" id="jwtvalue"  hidden=true  value="${AuthJWT}">
    ...
    <div class="clearfix">
      <button type="button" class="signupbtn" style='background-color: blue;' onclick="saveCustomerAnswer('submit')">submit</button>
      <button type="button" class="signupbtn" onclick="saveCustomerAnswer('save')">save for later</button> 
    </div>
  </div>
</form>
</body>
